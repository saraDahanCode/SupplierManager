...
import { completeOrder } from '../../redux/slices/orders.js';
...
export default function OrdersRowList() {
  const allOrders = useSelector(state => state.orders.ownerOrders);
  const uncompletedOrders = allOrders.filter(order => order.status === 'in process');
  const dispatch = useDispatch();

  if (!uncompletedOrders || uncompletedOrders.length === 0) {
    return (
  ...
  const handleCompleteAndRefresh = async (id) => {
    try {
  ...
  const rows = chunkArray(uncompletedOrders, 3);
    ...
    <Box sx={{ p: 2 }}>
      {rows.map((row, rowIndex) => {
        ...
        return (
          ...
          >
            {row.map(order => (
              <OrderCard key={order._id} order={order} onApprove={handleCompleteAndRefresh} />
            ))}
          </Box>
        );
      })}
